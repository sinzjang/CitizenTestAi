        ...
        # 리스트 표시(●)로 시작하는 답변인지 확인
            ...
            while j < len(lines):
                ...
                if (next_line and len(next_line) > 0 and not next_line[0].isupper() and not next_line.startswith('"')) or \
                   ...
                   next_line.startswith('House of Representatives') or next_line.startswith('la Cámara') or \
...
def extract_questions_and_answers(lines):
    ...
    while i < len(lines):
        ...
        # 서브카테고리 확인 (영어 버전만)
        if re.match(r'^[A-Z]:', line) and not any(spanish_word in line.lower() for spanish_word in ['principios', 'sistema', 'derechos', 'responsabilidades', 'período', 'años', 'historia', 'símbolos', 'días']):
...
def main():
    if len(sys.argv) != 3:
        print("사용법: python spanish_table_converter_v3.py <입력파일> <출력파일>")
        sys.exit(1)
    
    input_file = sys.argv[1]
    output_file = sys.argv[2]
    
    print("🚀 스페인어 시민권 시험 데이터 테이블 변환 시작 (v3)")
