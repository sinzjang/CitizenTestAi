...
def parse_french_csv(input_file, output_file):
    """프랑스어 병렬 텍스트를 표준 CSV 테이블로 변환"""
    ...
    while i < len(lines):
        ...
        if line.startswith('A:') or line.startswith('B:') or line.startswith('C:'):
            if not has_french(line):
                ...
                if i < len(lines) and lines[i].startswith(('A:', 'B:', 'C:')):
                    current_subcategory_fr = lines[i][3:].strip()
                    i += 1
                continue
            else:
                # 프랑스어 서브카테고리
        ...
        if question_match and not has_french(line):
            ...
            while i < len(lines):
                line = lines[i]
                # ●로 시작하거나 "●로 시작하는 경우
                if (line.startswith('●') or line.startswith('"●')) and not has_french(line):
                    # 따옴표 제거하고 답변 추가
            ...
            # 프랑스어 질문 찾기 (따옴표로 시작할 수도 있고, 공백이 없을 수도 있음)
            question_fr = ""
            if i < len(lines):
            ...
            # 프랑스어 답변 수집 (따옴표 안에 있을 수도 있음)
            answers_fr = []
            while i < len(lines):
                line = lines[i]
                # ●로 시작하거나 "●로 시작하는 경우
                if (line.startswith('●') or line.startswith('"●')) and has_french(line):
                    # 따옴표 제거하고 답변 추가
                    answer = line.lstrip('"').lstrip('●').strip().rstrip('"')
...
def main():
    print("=" * 60)
    print("🎯 프랑스어 병렬 텍스트 → 표준 CSV 테이블 변환")
    print("=" * 60)
    ...
    input_file = data_dir / '2025_CitizenTest_128 - French_Merged.txt'
    output_file = data_dir / '2025_CitizenTest_128 - French_Table.csv'
    
    # 변환
    questions = parse_french_csv(input_file, output_file)
