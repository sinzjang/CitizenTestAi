...
console.log('=== Spanish 파일 구조적 문제 상세 분석 ===\n');

// 날짜 패턴을 인식하여 콤마를 제외하는 스마트 카운팅 함수
function countOptionsSmartly(text) {
  if (!text) return 0;
  
  // 날짜 패턴들 (예: "July 4, 1776", "December 7, 1941" 등)
  const datePatterns = [
...
try {
  ...
  spanishData.forEach((question, index) => {
    ...
    if (hasProblems) {
      ...
      console.log(`❌ 질문 ${question.id}: "${(question.question_es || question.question).substring(0, 60)}..."`);
